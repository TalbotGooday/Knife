...
fun String.fromHtml(): Spanned {
	return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
		Html.fromHtml(this, Html.FROM_HTML_MODE_COMPACT, null, KnifeTagHandler())
	} else {
		Html.fromHtml(this, null, KnifeTagHandler())
	}
}
...
private fun withinHtml(out: StringBuilder, text: Spanned) {
	...
	while (i < text.length) {
		...
		if (styles.size == 2) {
			...
			when {
				styles[0] is BulletSpan -> withinBullet(out, text, i, next++)
				styles[0] is QuoteSpan -> withinQuote(out, text, i, next++)
